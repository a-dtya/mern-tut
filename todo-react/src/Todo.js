...
function Todo() {
    ...
    const handleComplete = async(id,data)=>{
      await databases.updateDocument("6465a1e5c4ed1e82b0f5","6465a1f120228e9beaf7",id,{
        todo:`${data} (Completed)`
      })
      window.location.reload()
    }
  return (
    <> {
      ...
      <div className="flex items-center justify-between mt-2">
              <button className='text-green-500 focus:outline-none' onClick={()=>handleComplete(item.$id,item.todo)}>Complete</button>
